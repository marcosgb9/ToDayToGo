...
func _physics_process(delta):
	...
	# --- MOVIMIENTO LATERAL ---
	if Input.is_action_pressed("mover_derecha_J1"):
		velocity.x = _velocidad
		animacion.flip_h = true
	elif Input.is_action_pressed("mover_izquierda_J1"):
		velocity.x = -_velocidad
	...
	# --- SALTO ---
	if Input.is_action_just_pressed("saltar_j1") and is_on_floor():
...
func _actualizar_animaciones():
	...
	if Input.is_action_just_pressed("atacar_j1"):
		if not is_on_floor():
			animacion.play("atacar_saltando")
		elif Input.is_action_pressed("agacharse_j1"):
